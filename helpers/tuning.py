...
def suggest_band_orders(current: dict, pool_presets: list,
    ...
    Ranking per band: healthy presets (fail==0) first, most ok first;
    untouched presets keep their current relative order; failing presets
    last, fewest failures first. Presets absent from the live pool are
    dropped; unknown pool presets are appended. Missing bands fall back to
    DEFAULT_BAND_ORDERS. Pure and total: never raises, covers all bands.
    """
