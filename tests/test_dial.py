...
# --- S13 (TDD) RED: fuzzy tier mapping for verbose preset names ---

def test_cost_promotes_names_with_fast_cheap_free():
    """Cost tier should match verbose names containing fast/cheap/free."""
    orders = {'light': ['Power', 'Default', 'Fast and Free', 'Very Fast but not very smart and Cheap']}
    out = dial.apply_dial(orders, 'cost')
    # Both cost-tier names should be promoted to the front
    assert out['light'][0] == 'Fast and Free'
    assert 'Very Fast but not very smart and Cheap' in out['light'][:2]


def test_quality_promotes_names_with_power_expensive_unhinged():
    """Quality tier should match verbose names containing power/expensive/unhinged."""
    orders = {'light': ['Fast and Free', 'Default', 'High Power and Free', 'Unhinged and Expensive']}
    out = dial.apply_dial(orders, 'quality')
    # power-containing names rank 0 (matching exact 'Power': 0), other
    # quality names rank 1, cost names tail — exact-map precedence kept
    assert out['light'][:3] == ['High Power and Free', 'Default',
                                'Unhinged and Expensive']


def test_unknown_names_fall_to_neutral_middle():
    """Preset names matching no tier stay in their original relative order in the middle."""
    orders = {'light': ['Unknown A', 'Power', 'Unknown B', 'Fast and Free']}
    out = dial.apply_dial(orders, 'cost')
    # Fast and Free promoted, unknowns keep relative order in the middle,
    # Power (quality tier via exact map) demoted last
    assert out['light'][0] == 'Fast and Free'
    assert out['light'][1:3] == ['Unknown A', 'Unknown B']
    assert out['light'][-1] == 'Power'
