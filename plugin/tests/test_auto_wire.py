...
# --- S4 (TDD) RED: pinned bands are frozen against auto-wire ---

def test_pinned_band_freezes_against_sync(tmp_path):
    p = _policy(tmp_path, orders={
        'light': ['Fast', 'Default'],
        'medium': ['Default', 'Fast'],
        'heavy': ['Power', 'Ghost'],
    }, extra={'pinned_bands': {'heavy': True}})
    rep = auto_wire.sync_band_orders(
        p, ['Default', 'Fast', 'Power', 'Storyteller'],
        tmp_path / 'wire-state.json')
    out = _read(p)['band_orders']
    # pinned heavy keeps its order verbatim (add and prune both skipped)
    assert out['heavy'] == ['Power', 'Ghost']
    # unpinned bands still sync
    assert out['light'] == ['Fast', 'Default', 'Storyteller']
    assert out['medium'] == ['Default', 'Fast', 'Storyteller']
    assert rep.pinned == ['heavy']
