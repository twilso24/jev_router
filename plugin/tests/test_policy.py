...
# --- S1 (TDD) RED: Decision.rule + Decision.reason_human on every path ---

def test_band_hit_sets_rule_and_human_reason():
    d = policy.resolve(_sig(comp=1.8), _entries())
    assert d.rule == 'band'
    assert 'Power' in d.reason_human
    assert 'heavy' in d.reason_human.lower()
    assert '[' not in d.reason_human and ']' not in d.reason_human


def test_fit_hit_sets_rule_and_human_reason():
    sig = _sig(comp=1.8)
    sig.preset_fit = 'Efficiency'
    sig.preset_fit_confidence = 0.9
    d = policy.resolve(sig, _entries())
    assert d.rule == 'fit'
    assert 'Efficiency' in d.reason_human
    assert '[' not in d.reason_human


def test_vision_filter_sets_rule_and_human_reason():
    d = policy.resolve(_sig(comp=1.8, vis=0.9), _entries())
    assert d.rule == 'vision-filter'
    assert d.compromise is True
    assert 'vision' in d.reason_human.lower()
...
def test_empty_pool_sets_empty_rule():
    d = policy.resolve(_sig(), [])
    assert d.rule == 'empty'
    assert d.entry is None
