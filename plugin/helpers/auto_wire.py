    ...
    try:
        ...
        for p in (pool_presets or []):
            ...
            if sp and sp not in seen:
                ...
                pool.append(sp)
        orders = load_band_orders(policy_path)
        pinned = tuning.read_pinned_bands(policy_path)
        ...
        pruned: list = []
        pruned_seen = set()
        pinned_set = {b for b, v in (pinned or {}).items() if v}
        new_orders = {}
        for band, names in orders.items():
            if band in pinned_set:
                # pinned band: frozen verbatim, no adds or prunes
                new_orders[band] = [str(n) for n in names]
                continue
            kept = []
            for n in names:
                sn = str(n)
        ...
        if added or pruned:
            for band in new_orders:
                if band in pinned_set:
                    continue  # pinned: tail-append frozen too
            ...
            if not ok:
                return empty
            report = WireReport(added=added, pruned=pruned,
                                pinned=sorted(pinned_set), ts=time.time())
