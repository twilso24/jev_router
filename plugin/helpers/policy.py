...
def resolve(sig: Signals, entries: list[PoolEntry],
    ...
    if not entries:
        return Decision(
            None, band, 'empty pool: no eligible entries', rule='empty',
    ...
    if honor_fit and sig.preset_fit:
        ...
        if (confident and fit_entry is not None
            ...
            reason = (f'task_class={sig.task_class} band={band} -> '
                      f'{chosen} [fit]')
            human = (f'Best match: {fit_entry.preset_name} '
                     f'({fit_entry.provider}/{fit_entry.model}) - '
                     f'Jev fit pick for this {sig.task_class} task')
            return Decision(fit_entry, band, reason,
                            wanted=wanted[0], fit_used=True,
                            rule='fit', reason_human=human)
    ...
    for name in wanted:
        ...
        if deviated_vision:
            ...
            human = (f'Routed to {entry.preset_name} '
                     f'({entry.provider}/{entry.model}): this task needs '
                     f'a vision-capable model')
            return Decision(entry, band, reason, compromise=True,
                            wanted=wanted[0], rule='vision-filter',
                            reason_human=human)
        if deviated_missing:
            reason = f'preferred presets unavailable for band={band} -> {chosen}'
            human = (f'Routed to {entry.preset_name} '
                     f'({entry.provider}/{entry.model}): preferred presets '
                     f'unavailable for {band} tasks')
            return Decision(entry, band, reason, wanted=wanted[0],
                            rule='band-fallback', reason_human=human)
        reason = f'task_class={sig.task_class} band={band} -> {chosen}'
        human = (f'Routed to {entry.preset_name} '
                 f'({entry.provider}/{entry.model}): {band} '
                 f'{sig.task_class} task')
        return Decision(entry, band, reason, wanted=name,
                        rule='band', reason_human=human)
    ...
    if vision_hard:
        ...
        if not capable:
            return Decision(
                ...
                rule='empty',
                reason_human='No vision-capable preset available; '
                             'keeping the active preset model')
        first = capable[0]
        ...
        human = (f'Routed to {first.preset_name} '
                 f'({first.provider}/{first.model}): vision required and '
                 f'preferred presets unavailable')
        return Decision(first, band, reason, compromise=True,
                        wanted=wanted[0], rule='band-fallback',
                        reason_human=human)
    reason = (
    ...
    human = (f'Routed to {first.preset_name} '
             f'({first.provider}/{first.model}): preferred presets '
             f'unavailable for {band} tasks')
